...
Based on: https://github.com/materaj/nmap-parser-xml-to-csv
...
def NmapXmlParser(filenames, options):
    ...
    ### Remove all hostname(user,ptr) to be just user type only.
            ...
            for port in hosttag.getElementsByTagName('port'):
                ...
                row_port = row_port + str(port.getAttribute('portid')) + ", "
                
                for state in port.getElementsByTagName('state'):
                    ##row.append(state.getAttribute('state'))
                    row_port = row_port + str(state.getAttribute('state')) + ", "
                
                for service in port.getElementsByTagName('service'):
                    row_port = row_port + str(service.getAttribute('name')) + ", "
                
                if port.getElementsByTagName('script'):
                    scriptFound = True
                    for script in port.getElementsByTagName('script'):
                        row_port = row_port + (script.getAttribute('id')) + ", "
                        row_port = row_port + (repr(script.getAttribute('output').encode('ascii')))# .replace('\n  ','')
                #row_port = row_port + ('\n')
                row.append(row_port)
            
            if hosttag.getElementsByTagName('port'):
                oOuput.Row(row)
...
wildcards are supported'''
    ...
    oParser = optparse.OptionParser(usage='usage: %prog [options] [@]file ...\n \n Example: python nmap_xml_to_csv.py -o nmap.csv *.xml')
